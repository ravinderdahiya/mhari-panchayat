...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _resendOtp() async {
    if (_isSubmitting || _isResending || _resendSecondsRemaining > 0) return;

    final mobile = _mobileController.text.trim();
    setState(() => _isResending = true);
    try {
      final result = await AuthApi.resendOtp(mobile);
      if (!mounted) return;
      _clearOtp();
      _startOtpCountdown(
  ...
  Future<void> _continueAsStaff() async {
    ...
    if (staffId.isEmpty || password.isEmpty) {
      _showMessage(
        _t('यूज़र ID और पासवर्ड दर्ज करें', 'Enter User ID and Password'),
      );
      return;
    }
    ...
    try {
      ...
      if (!_otpSent) {
        ...
        if (_otpSecondsRemaining == 0) {
          _showMessage(
            _t(
              'ओटीपी की समय-सीमा समाप्त हो गई है। नया ओटीपी भेजें।',
        ...
        if (otp.length != 4) {
          _showMessage(
            _t('कृपया पूरा ओटीपी दर्ज करें', 'Please enter the complete OTP'),
          );
          return;
        }
  ...
  List<Widget> _buildStaffForm() {
    return [
      Text(
        _t(
          'मोबाइल, Emp ID, ईमेल या यूज़र ID और पासवर्ड से लॉगिन करें',
          'Login with mobile, Emp ID, email or User ID and Password',
        ),
        textAlign: TextAlign.center,
        style: GoogleFonts.notoSansDevanagari(
          fontSize: 13,
      ...
      const SizedBox(height: 14),
      _buildStaffField(
        label: _t('मोबाइल / Emp ID / ईमेल', 'Mobile / Emp ID / Email'),
      ...
      Align(
        ...
        child: TextButton(
          ...
          child: Text(
            _t('पासवर्ड भूल गए?', 'Forgot password?'),
            style: GoogleFonts.notoSansDevanagari(
              fontSize: 12.5,
  ...
  Widget _buildOtpStatus() {
    ...
    final canResend = !_isResending && _resendSecondsRemaining == 0;
    ...
    return Row(
      children: [
        Icon(
          ...
          size: 17,
          color: expired ? Colors.red.shade700 : AppColors.secondaryText,
        ),
        const SizedBox(width: 5),
        Expanded(
          child: Text(
            expired
                ? _t('ओटीपी समाप्त', 'OTP expired')
                : _t('ओटीपी की समय-सीमा: $time', 'OTP expires in $time'),
            style: GoogleFonts.notoSansDevanagari(
              fontSize: 12,
              fontWeight: FontWeight.w600,
              color: expired ? Colors.red.shade700 : AppColors.secondaryText,
          ...
          child: _isResending
              ...
              : Text(
                  _resendSecondsRemaining > 0
                      ...
                      : _t('ओटीपी पुनः भेजें', 'Resend OTP'),
                  style: GoogleFonts.notoSansDevanagari(
                    fontSize: 12,
                    fontWeight: FontWeight.w700,
                    color: canResend
            ...
            suffixIcon: obscure
                ? IconButton(
                    tooltip: _passwordVisible
                        ? _t('पासवर्ड छिपाएं', 'Hide password')
                        : _t('पासवर्ड दिखाएं', 'Show password'),
                    onPressed: () =>
...
// The gradient banner above the login card: language toggle, the brand mark
// in a ring-and-badge frame, app name, and a locality subtitle. Cut off at
// the bottom with rounded corners so the two soft "blob" tints peek out
// clipped, rather than as visible circles.
        ...
        child: Stack(
          ...
          children: [
            Positioned(
              top: -30,
              right: -40,
              child: _Blob(color: AppColors.secondary.withValues(alpha: 0.18), size: 140),
            ),
            Positioned(
              bottom: -60,
              left: -50,
              child: _Blob(color: AppColors.primary.withValues(alpha: 0.12), size: 170),
            ),
            Column(
              children: [
                Align(
